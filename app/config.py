...
class Settings(BaseSettings):
    # ── Keycloak OIDC (public PKCE client — no client secret) ──────────────
    ...
    # No client_secret — public PKCE clients use code_verifier as proof of identity

    # ── Keycloak Admin (confidential service account — client credentials) ──
    # Create a confidential client in the master realm with:
    #   - Service accounts roles: ON
    #   - Service account role: realm-management → realm-admin
    keycloak_admin_client_id: str = Field(
        default="keycloak-chatbot-backend", env="KEYCLOAK_ADMIN_CLIENT_ID"
    )
    keycloak_admin_client_secret: str = Field(
        default="", env="KEYCLOAK_ADMIN_CLIENT_SECRET"
    )
    ...
    # ── Derived URLs (computed from stored fields, never read from .env) ────
